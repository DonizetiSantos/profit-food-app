...
export const FinancialAssumptions: React.FC<Props> = ({ banks }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Fetch all payment methods for the company to ensure every method has a rule
        ...
        // Create default rules for methods that don't have one yet
        ...
        // Combine and sort by payment method name
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      // Remove the joined data before saving
    ...
    <div className="space-y-8 animate-fade-in">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Configure as regras de recebimento e taxas para cada meio de pagamento.</p>
        </div>
        
        <button 
          onClick={handleSave}
          disabled={saving || rules.length === 0}
          className={`flex items-center gap-2 px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-xl ${saving || rules.length === 0 ? 'bg-slate-800 text-slate-600 cursor-not-allowed' : 'bg-rose-600 hover:bg-rose-500 text-white shadow-rose-600/20'}`}
        >
          {saving ? (
            <>
      ...
      <div className="bg-slate-900 rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl">
        {rules.length === 0 ? (
          <div className="py-32 text-center px-8">
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2">Nenhuma regra encontrada</h3>
                  ...
                  <tr key={rule.id} className="hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-8 py-5 text-center">
                      <input 
                        ...
                        onChange={(e) => handleUpdateRule(rule.id, 'settlement_days', parseInt(e.target.value) || 0)}
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <button 
                          ...
                          className={`w-12 h-6 rounded-full p-1 transition-all duration-300 relative ${rule.receives_same_day ? 'bg-rose-600' : 'bg-slate-800'}`}
                        >
                        ...
                        {rule.receives_same_day && (
                          <span className="text-[8px] font-black text-rose-500 uppercase tracking-tighter">D+0</span>
                        )}
                      </div>
                    </td>
                    <td className="px-8 py-5 text-center">
