...
const isAutoRetainedPdvFee = (posting: FinancialPosting) => {
  const observations = normalizeText(posting.observations);

  return (
    posting.group === MainGroup.DESPESAS &&
    observations.startsWith('TAXA PDV')
  );
};

const isDashboardPayableExpense = (posting: FinancialPosting) => {
  if (posting.group !== MainGroup.DESPESAS) return false;
  if (isAutoRetainedPdvFee(posting)) return false;
  return true;
};
